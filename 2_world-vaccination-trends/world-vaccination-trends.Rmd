...
#upload dataset
world_vaccination <- read_csv(here("2_world-vaccination-trends", "data", "world_vaccination.csv"))

world_vaccination <- filter(world_vaccination, !is.na(pct_vaccinated), who_region !=("(WHO) Global"))
...
polio_regions_line <- polio %>% 
  ggplot(aes(x = yr, y = pct_vaccinated, color = who_region, shape = who_region)) +
  geom_line() +
  labs(x = "Year", y = "% People Vaccinated", title = "% of People Vaccinated Against Polio Each Year", color = "Regions")
...
```{r}
#filtering hepB
HepB <- world_vaccination %>% 
  filter(vaccine == "hepatitis_b")
#plot 
HepB_regions_line <- HepB %>% 
  ggplot(aes(x = yr, y = pct_vaccinated, color = who_region, shape = who_region)) +
  geom_line() +
  labs(x = "Year", y = "% People Vaccinated", title = "% of People Vaccinated Against HepB Each Year", color = "Regions")

print(HepB_regions_line)

#combining plots 

polio_and_hepB <- world_vaccination %>% 
  ggplot(aes(x = yr, y = pct_vaccinated, color = who_region, shape = who_region)) +
  geom_line() +
  labs(x = "Year", y = "% People Vaccinated", title = "% of People Vaccinated Against HepB Each Year", color = "Regions")+
  facet_wrap(~vaccine)

print(polio_and_hepB)
